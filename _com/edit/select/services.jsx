...
import { ModifyServices } from "@/_lib/listings/editdata";

export default function EditServices({services, allservices, listing_id, handleEdit, setListing}) {
    
    const [selected, setSelected] = useState(services.map((e)=>e.name));
    ...
    async function HandleMaplink(){
        setLoading(true);
        setError('');
        const result = await ModifyServices(listing_id, selected);   
        setLoading(false)
